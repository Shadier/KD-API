...
exports.productRouter.get('/', (req, res) => {
    Product.find((err, products) => {
        ...
        if (!products)
            return res.status(404).send({ message: 'products not founded!' });
...
exports.productRouter.post('/', (req, res) => {
    ...
    if (params.codebar && params.name && params.cost && params.price) {
        ...
        Product.find({ codebar: product.codebar.toLowerCase() }).exec((err, products) => {
            if (err)
                return res.status(500).send({ message: 'Internal Server error, product doesn´t saved' });
            if (products && products.length >= 1)
                return res.status(200).send({ message: "codebar registered before" });
            else {
                product.save((err, productStored) => {
                    ...
                    if (productStored)
                        ...
                        res.status(404).send({ message: 'product not saved!' });
                });
            }
        });
    }
...
exports.productRouter.patch('/:id', (req, res) => {
    const productId = req.params.id;
    const params = req.body;
    Product.update({ _id: productId }, { $set: params }, (err, productUpdated) => {
        if (err)
            return res.status(500).send({ message: 'Internal Server error, product doesn´t updated' });
        if (productUpdated)
...
exports.productRouter.delete('/:id', (req, res) => {
    ...
    Product.findByIdAndRemove(productId, (err, productDeleted) => {
        ...
        if (productDeleted)
            res.status(200).send({ messafe: 'product Deleted successfully!' });
        else
