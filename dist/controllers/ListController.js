...
exports.listRouter.get('/', (req, res) => {
    List.find((err, lists) => {
        ...
        if (!lists)
            return res.status(404).send({ message: 'lists not founded!' });
        return res.status(200).send({ lists });
    });
});
exports.listRouter.get('/:id', (req, res) => {
    const listId = req.params.id;
    List.findById(listId, (err, list) => {
        if (err)
            return res.status(500).send({ message: 'Internal Server error' });
        if (!list)
            return res.status(404).send({ message: 'List not founded!' });
        return res.status(200).send({ list });
    });
});
exports.listRouter.patch('/:id', (req, res) => {
    const listId = req.params.id;
    ...
    if (params.productId) {
        Product.findById(params.productId, (err, product) => {
            ...
            if (!product)
                return res.status(400).send({ message: "that product doesnt exists" });
            else {
                params.product = product;
                List.findByIdAndUpdate(listId, params, (err, listUpdated) => {
                    if (err)
                        return res.status(500).send({ message: 'Internal Server error, List doesn´t updated' });
                    if (listUpdated)
                        res.status(200).send({ messafe: 'List updated successfully!' });
                    else
                        res.status(404).send({ message: 'List not updated!' });
                });
            }
        });
    }
...
exports.listRouter.delete('/:id', (req, res) => {
    const listId = req.params.id;
    const params = req.body;
    List.findByIdAndRemove(listId, (err, listDeleted) => {
        ...
        if (listDeleted)
            res.status(200).send({ messafe: 'List Deleted successfully!' });
        else
            res.status(404).send({ message: 'List not Deleted!' });
    });
});
exports.listRouter.post('/', (req, res) => {
    const params = req.body;
    let list = new List();
